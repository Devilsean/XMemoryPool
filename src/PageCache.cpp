...
Span *PageCache::allocSpan(size_t numPages) {
  ...
  while (true) {
    ...
    if (k >= MAX_PAGES) {
      ...
      Span *span =
          (Span *)std::malloc(sizeof(Span)); // 使用 malloc 避开重写的 new
      new (span) Span();
...
void PageCache::deallocSpan(Span *span) {
  ...
  while (true) {
    ...
    if (span->numPages + prevSpan->numPages >= MAX_PAGES)
      break;
  ...
  while (true) {
    ...
    if (nextSpan == nullptr || nextSpan->inUse == true ||
        nextSpan->pageId != nextId)
      break;

    if (span->numPages + nextSpan->numPages >= MAX_PAGES)
      break;
